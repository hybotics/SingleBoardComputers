...
void loop()
{
  ...
  if (BLUNOPlainProtocol.available()) {
    ...
    else if (BLUNOPlainProtocol.receivedCommand.startsWith("DISP")){
      fill(0x00);
      char* charBuffer = (char*)malloc(BLUNOPlainProtocol.receivedCommand.substring(strlen("DISP")).length()+1);
      BLUNOPlainProtocol.receivedCommand.substring(strlen("DISP")).toCharArray(charBuffer, BLUNOPlainProtocol.receivedCommand.substring(strlen("DISP")).length()+1);
      if (BLUNOPlainProtocol.receivedContentLenth) {
        show_string(BLUNOPlainProtocol.receivedContent[0],BLUNOPlainProtocol.receivedContent[1],charBuffer);
      }
      else{
        show_string(0,0,charBuffer);
      }
      free(charBuffer);
    ...
    {
      BLUNOPlainProtocol.write("KNOB", 1023-analogRead(KnobPin));
    }
  }
